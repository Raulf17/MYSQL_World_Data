...
INSERT INTO `country` VALUES ('SUR','Suri`Name`','South America','South America',163265.00,1975,417000,71.4,870.00,706.00,'Suri`Name`','Republic','Ronald Venetiaan',3243,'SR');
...
COMMIT;
------------------- 'CREATE TABLE COUNTRY LANGUAGES' ---------------------------------------
...
----------------------- 'INSERT VALUES INTO COUNTRY LANGUAGES TABLE' --------------------------------------
...
INSERT INTO `countrylanguage` VALUES ('AUS','Viet`Name`se','F',0.8);
...
INSERT INTO `countrylanguage` VALUES ('KHM','Viet`Name`se','F',5.5);
...
INSERT INTO `countrylanguage` VALUES ('USA','Viet`Name`se','F',0.2);
...
INSERT INTO `countrylanguage` VALUES ('VNM','Viet`Name`se','T',86.8);
...
SELECT continent, `Name` as country
FROM world.country
...
# identifing duplicates with aggregate functions #
...
-- #1. in the world.city, What are the cities & country codes of all locations in the database ( Sorted alphabetically)
SELECT `Name` As City, CountryCode
FROM world.city
ORDER BY CountryCode, city desc;

-- #2. Return the `Name`s of all cities containted in the world.city that in the united states

SELECT `Name` AS City
FROM world.city
WHERE CountryCode = 'USA';

-- #3.Find all the total population of cities contained in the world.city that are in the united states.

SELECT SUM(Population) AS USA_population
FROM world.city
WHERE CountryCode = 'USA';

-- #4. return the `Name`s of all cities contained in the world.city table that are in Nebraska #(Region), with subquery.

SELECT subquery.city
FROM ( SELECT `Name` AS city, District FROM world.city WHERE District = 'Nebraska' AND CountryCode = 'USA' ) AS subquery;

-- #5. In the world.country language table,  select all countrycodes where the offical language is 'english'

SELECT *
FROM world.countrylanguage
WHERE Language like 'english%';

-- #6. In the world.country table, create an “average life expectancy” grouping, then count the number of countries in each group. #(using a CASE statement)

SELECT COUNT(*),
  CASE
    WHEN LifeExpectancy BETWEEN 40 AND 49 THEN '40-49'
    ...
    ELSE 'Other'
  END AS Life_Exp_Group
FROM world.country
GROUP BY Life_Exp_Group;

----------------------------------- # CASE STATEMENTS # ----------------------------------

-- What is the total world population by continent 
SELECT distinctrow(continent)
FROM COUNTRY;

SELECT sum(Population) AS total_pop,
CASE 
	WHEN Continent IN ('Asia', 'Africa', 'Eurpoe', 'Oceania') THEN 'Eastern'
    WHEN RIGHT(Continent,7) = 'America' then ' Western'
	ELSE continent 
    END AS Hemisphere
    FROM world.country
    GROUP BY Hemisphere;
...
ON a.Code = b.CountryCode and b.IsOfficial='T');
